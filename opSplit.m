function [U,x,t]=opSplit(nX,nT,X,T)
...
nC=length(t);            % length of time vector
...
U(1*nR+1:2*nR,:)=ni*ones(nR,nC); % solutions for n
U(2*nR+1:3*nR,:)=mi*ones(nR,nC); % solutions for m
U(3*nR+1:4*nR,:)=hi*ones(nR,nC); % solutions for h
