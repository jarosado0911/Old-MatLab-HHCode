function [U,x,t]=opSpltB(nX,nT,x0,xf,t0,tf)

%default
if nargin==0
    [nX, nT,x0, xf, t0, tf]=deal(180,100000,0,1,0,9);
end
...
c=0.9;%0.005; %c=0.9e-2; %0.009 % capacitance
...
for j=1:nTime-1
    % Diffusion Solve
   [Uout,~,~]=diffSolve(U(1:nR,j),U(1,j+1),U(nR,j+1),b,nX,3,x0,xf,t(j),t(j+1));
   tmpU=Uout(:,end);
   
    %Now Solve Reaction Equation using forward euler
          U(2:nR-1,j+1)=tmpU(2:nR-1)+k*f1(U(nR+2:2*nR-1,j),U(2*nR+2:3*nR-1,j),U(3*nR+2:4*nR-1,j),tmpU(2:nR-1));
